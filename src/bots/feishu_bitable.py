...
class FeishuBitable:
    ...
    ARTICLE_FIELDS = [
        ...
        {"field_name": "云文档", "type": 15},  # 15=URL 飞书文档链接
        {"field_name": "用户反馈", "type": 1},  # 1=文本 用户反馈（有用/没用/收藏）
    ]
    ...
    def _convert_article_to_record(self, article: dict) -> dict:
        ...
        # 只更新提供的字段，未提供的字段保持不变（飞书 API 行为）
        # 标题
        if 'title' in article:
            fields["标题"] = article.get('title', '')[:1000]

        # 链接
        if 'url' in article:
            fields["链接"] = {"link": article.get('url', ''), "text": "原文链接"}
        ...
        if 'summary' in article:
            fields["摘要"] = (article.get('summary', '') or '')[:2000]

        # 中文摘要
        if 'zh_summary' in article:
            fields["中文摘要"] = (article.get('zh_summary', '') or '')[:2000]

        # 优先级
        if 'priority_score' in article or 'priority' in article:
        ...
        if 'fetched_at' in article:
            ...
            if fetched_at:
                ...
                fetched_at_ts = int(datetime.now().timestamp() * 1000)
            fields["抓取时间"] = fetched_at_ts

        # 已推送
        if 'is_pushed' in article:
            fields["已推送"] = article.get('is_pushed', False)

        # 云文档链接
        if 'feishu_doc_url' in article:
            feishu_doc_url = article.get('feishu_doc_url', '')
            fields["云文档"] = {"link": feishu_doc_url, "text": "飞书文档"} if feishu_doc_url else None

        # 用户反馈
        if 'user_feedback' in article:
            user_feedback = article.get('user_feedback', '')
            fields["用户反馈"] = user_feedback
