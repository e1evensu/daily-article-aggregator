...
class PWCFetcher(BaseFetcher):
    ...
    def fetch(self) -> FetchResult:
        ...
        return FetchResult(
            items=items, source_name="Papers With Code", source_type="pwc"
        )
    ...
    def _fetch_from_api(self) -> list[dict[str, Any]]:
        ...
        try:
            logger.info(
                f"Fetching papers from Papers With Code (limit={self.limit})..."
            )
            ...
            response = requests.get(
                papers_url, params=params, headers=headers, timeout=self.timeout
            )
            ...
            if "application/json" not in content_type:
                logger.warning(
                    f"PWC API returned non-JSON content-type: {content_type}"
                )
                return []
            ...
            try:
                ...
                logger.error(f"PWC API returned invalid JSON: {e}")
                return []
            except ValueError as e:
                logger.error(f"PWC API returned invalid JSON: {e}")
                return []
    ...
    def _parse_paper(self, paper: dict) -> dict[str, Any] | None:
        ...
        if published_date:
            # 只保留日期部分
            published_date = (
                published_date[:10] if len(published_date) >= 10 else published_date
            )
...
def parse_pwc_paper(paper: dict) -> dict[str, Any] | None:
    ...
    # 提取论文 URL
    paper_url = (
        paper.get("url_abs", "") or paper.get("url_pdf", "") or paper.get("url", "")
    )
    if not paper_url:
        paper_id = paper.get("id", "")
        if paper_id:
