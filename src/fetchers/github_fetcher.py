...
class GitHubFetcher(BaseFetcher):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.topics = self.config.get(
            "topics", ["security", "llm", "ai", "machine-learning"]
        )
    ...
    def _search_by_topic(self, topic: str) -> list[dict[str, Any]]:
        """按话题搜索项目"""
        cutoff_date = (datetime.now() - timedelta(days=self.days_back)).strftime(
            "%Y-%m-%d"
        )
        ...
        try:
            ...
            data = response.json()
            if data is None:
                logger.warning("GitHub API returned None for new trending search")
                return []
            items = data.get("items", [])
        except requests.exceptions.RequestException as e:
            logger.error(f"GitHub API request failed for new trending: {e}")
            return []
        except Exception as e:
            logger.error(f"Unexpected error searching new trending: {e}")
            return []
