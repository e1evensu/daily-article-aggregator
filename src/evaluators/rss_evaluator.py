...
def _parse_feed_with_timeout(url: str, timeout: int = 30) -> Any:
    """
    带超时控制的 feedparser.parse() 包装函数

    feedparser.parse() 本身不支持超时参数，当网络连接卡住时会导致无限等待。
    此函数使用 socket 设置全局超时，确保网络操作不会无限期阻塞。
...
class RSSEvaluator:
    ...
    def __init__(self, ai_analyzer: Any, config: dict):
        ...
        logger.info(
            f"RSSEvaluator initialized: inactive_months={self.inactive_months}, "
    ...
    def check_feed_activity(self, url: str) -> tuple[bool, str]:
        ...
        **验证: 需求 9.1, 9.2**
        """
        try:
            feed = _parse_feed_with_timeout(url, timeout=self.timeout)
            ...
            if last_updated_dt:
                ...
                logger.info(
                    f"订阅源 {url} 最后更新: {last_updated_str}, 活跃: {is_active}"
                )
        ...
        **验证: 需求 9.5**
        ...
        try:
            ...
            prompt = TECHNICAL_DEPTH_PROMPT.format(
                title=title or "", content=truncated_content
            )
    ...
    def _calculate_quality_score(
        self, is_active: bool, originality_score: float, technical_depth: str
    ) -> float:
        """
        计算综合质量评分
        ...
        **验证: 需求 9.6**
        ...
        # 加权计算
        quality_score = (
            activity_score * 0.3 + originality_score * 0.4 + depth_score * 0.3
        )
    ...
    def evaluate_feed(self, url: str) -> FeedEvaluation:
        ...
        **验证: 需求 9.1-9.7**
        ...
        avg_originality = (
            sum(originality_scores) / len(originality_scores)
        ...
        technical_depth = self.evaluate_technical_depth(
            first_article.get("title", ""), first_article.get("content", "")
        )
        ...
        quality_score = self._calculate_quality_score(
            is_active, avg_originality, technical_depth
        )
        ...
        logger.info(
            f"订阅源 {feed_name} 评估完成: score={quality_score}, recommendation={recommendation}"
        )
        return result

    def _evaluate_feed_with_timeout(
    ...
    def evaluate_all_feeds(
        ...
        logger.info(
            f"从OPML文件解析出 {len(urls)} 个订阅源，并发数: {concurrency}，超时: {feed_timeout}s"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
            ...
            future_to_url = {
                executor.submit(
                    self._evaluate_feed_with_timeout, url, feed_timeout
                ): url
            ...
            for future in concurrent.futures.as_completed(future_to_url):
                ...
                try:
                    ...
                    logger.info(
                        f"评估进度: {processed_count}/{total} - {evaluation.name[:30]}... ({evaluation.recommendation})"
                    )
                except Exception as e:
    ...
    def generate_report(self, evaluations: list[FeedEvaluation]) -> str:
        ...
        **验证: 需求 9.8**
        ...
        avg_score = (
            sum(e.quality_score for e in evaluations) / total if total > 0 else 0
        )
        ...
        report_lines = [
            ...
            "",
            f"- 总订阅源数: {total}",
            f"- 活跃订阅源: {active_count} ({active_count / total * 100:.1f}%)"
            if total > 0
            else "- 活跃订阅源: 0",
        ...
        if review_feeds:
            ...
            if normal_review:
                ...
                for e in sorted(
                    normal_review, key=lambda x: x.quality_score, reverse=True
                ):
                    categories_str = (
                        ", ".join(e.categories) if e.categories else "未分类"
                    )
                    report_lines.extend(
                        [
                            f"- **{e.name}**",
        ...
        if remove_feeds:
            ...
            for e in remove_feeds:
                reason = (
                    "不活跃" if not e.is_active else f"质量评分过低 ({e.quality_score})"
                )
                report_lines.extend(
                    [
                        f"- **{e.name}**: {reason}",
    ...
    def export_filtered_opml(
        self,
        evaluations: list[FeedEvaluation],
        output_path: str,
        min_score: float = 0.6,
    ) -> int:
        """
        ...
        **验证: 需求 9.8**
        ...
        # 筛选符合条件的订阅源
        filtered_feeds = [
            e for e in evaluations if e.is_active and e.quality_score >= min_score
        ]
        ...
        try:
            ...
            logger.info(
                f"导出筛选后的OPML文件: {output_path}, 保留 {len(filtered_feeds)} 个订阅源"
            )
        except Exception as e:
